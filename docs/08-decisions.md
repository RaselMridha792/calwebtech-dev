...
| 12 | 2026-09-13 | `pnpm lh` runs Lighthouse CI three times against the built API and web app on a seeded database and asserts on the median run. The gates are 90+ in every category, LCP under 2.5s, CLS under 0.1, Total Blocking Time under 200ms (the lab proxy for INP) and script transfer under 150KB. The page and image caches are warmed first. Campaign pages skip the `is-crawlable` audit because they are noindex by design. | These are the budget gates from `CLAUDE.md`, measured the way production serves the page. |
