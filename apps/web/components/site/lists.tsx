...
export function EmptyState({
  ...
  return (
    <div
      className={`rounded-2xl border border-dashed px-6 py-10 text-center text-[15px] ${dark ? 'border-white/25 text-white/75' : 'border-line bg-white text-body'} ${className}`}
    >
      <p>{children}</p>
      {action ? (
        <a
          href={action.href}
          className={`mt-4 inline-block py-1 font-semibold ${dark ? 'text-white underline underline-offset-4' : 'text-primary hover:text-primaryd'}`}
        >
          {action.label}
        </a>
      ) : null}
    ...
    <ul className={layout}>
      {items.map((item) => (
        <li key={item} className="flex items-start gap-3.5 text-ink">
          <span
            className="mt-0.5 grid h-6 w-6 shrink-0 place-items-center rounded-full bg-result/15 text-result"
            aria-hidden="true"
          >
  ...
  return (
    <ol className={`grid gap-6 md:grid-cols-2 ${STEP_COLUMNS[steps.length] ?? 'lg:grid-cols-3'}`}>
      {steps.map((step, index) => (
        <li
          key={step.title}
          className={`border-t-2 pt-5 ${index === 0 ? (dark ? 'border-white' : 'border-ink') : dark ? 'border-white/20' : 'border-line'}`}
          {...reveal(index)}
        >
          ...
          <p className={`mt-2.5 text-[15px] leading-relaxed ${dark ? 'text-white/75' : ''}`}>{step.body}</p>
          <p className={`mt-3 text-[13.5px] font-medium ${dark ? 'text-white/70' : ''}`}>{step.duration}</p>
        </li>
      ))}
    </ol>
  );
...
export function Prose({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div
      className={`max-w-[72ch] text-[17px] leading-relaxed [&_a]:font-semibold [&_a]:text-primary [&_a]:underline [&_a]:underline-offset-4 [&_a:hover]:text-primaryd [&_h2]:mt-12 [&_h2]:font-display [&_h2]:text-[28px] [&_h2]:leading-tight [&_h2]:font-extrabold [&_h2]:text-ink [&_h3]:mt-8 [&_h3]:font-display [&_h3]:text-[20px] [&_h3]:font-bold [&_h3]:text-ink [&_li]:mt-2 [&_ol]:mt-4 [&_ol]:list-decimal [&_ol]:pl-6 [&_p]:mt-4 [&_strong]:text-ink [&_ul]:mt-4 [&_ul]:list-disc [&_ul]:pl-6 [&>:first-child]:mt-0 ${className}`}
    >
      {children}
    </div>
  );
