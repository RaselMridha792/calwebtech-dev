...
export function ContactDetailsCard({ view }: { view: StaticContactView }) {
  ...
  return (
    <div className="border border-hairline bg-canvas-raised p-7 text-ink-muted sm:p-8">
      ...
      <p className="mt-6 border-t border-hairline pt-5 text-[15px] leading-relaxed">{details.response}</p>
    </div>
  );
}
          ...
          {offices.length > 0 ? (
            <div>
              ...
              <ul className="mt-4 grid gap-5 sm:grid-cols-2">
                {offices.map((office) => (
                  <li key={office.city} className="border border-hairline bg-canvas-raised p-5">
    ...
    <Section id="routing" tone="mist" labelledBy="routing-heading">
      ...
      <ul className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {view.enquiryTypes.map((type, index) => (
          <li key={type.slug} className="flex flex-col border border-hairline bg-canvas-raised p-7" {...reveal(index)}>
            ...
            <a
              ...
              className="mt-auto inline-block self-start pt-5 font-semibold text-gold-ink hover:text-gold-600"
            >
              {routing.linkLabel}
