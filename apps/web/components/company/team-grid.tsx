    ...
    <ul data-company-list="team" className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
      {members.map((member, index) => (
        <li
          key={member.slug}
          data-team-member=""
          className="lift flex flex-col overflow-hidden border border-hairline bg-canvas-raised"
          {...reveal(index)}
        >
          {member.photo ? (
          ...
          <div className="flex flex-1 flex-col p-6">
            ...
            <p className="text-[14.5px] font-semibold text-gold-ink">{member.role}</p>
            ...
            {member.skills.length > 0 ? (
              <ul aria-label={`What ${member.name} works on`} className="mt-4 flex flex-wrap gap-2">
                {member.skills.map((skill) => (
                  <li key={skill} className="bg-canvas-sunken px-2.5 py-1 text-[12.5px] font-medium text-ink">
                    {skill}
                  </li>
                ))}
              </ul>
