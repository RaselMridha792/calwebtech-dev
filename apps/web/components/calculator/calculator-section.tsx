    ...
    <Section id={CALCULATOR_ANCHORS.tool} tone="tint" labelledBy={headingId} deferred={false}>
      <div className="grid gap-12 lg:grid-cols-12 lg:gap-16">
        <div className="lg:col-span-5">
          ...
          <noscript>
            <p className="max-w-[46ch] rounded-xl border border-line bg-white p-5 text-[15px] leading-relaxed">
  ...
  return (
    <div className="rounded-2xl border border-line bg-white p-6 text-ink shadow-panel sm:p-9">
      ...
      <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-mist" aria-hidden="true">
        <span className="block h-full rounded-full bg-primary" style={{ width: `${String(Math.round(100 / (total + 1)))}%` }} />
      </div>
      ...
      <ul className="mt-6 space-y-3">
        {step.options.map((option) => (
          <li
            key={option.value}
            className="flex items-start gap-3.5 rounded-xl border border-line px-5 py-3.5 text-[15.5px]"
          >
            <span className="mt-1 h-4 w-4 shrink-0 rounded-full border-2 border-line" aria-hidden="true" />
            <span className="min-h-6">
              {option.label}
              {option.description ? (
                <span className="mt-1 block text-[14px] leading-relaxed text-body">{option.description}</span>
              ) : null}
            </span>
          </li>
        ))}
      ...
      <p data-calculator-start="" className="mt-7">
        <button
          type="button"
          className="inline-flex h-12 items-center justify-center rounded-xl bg-primary px-6 text-[15.5px] font-semibold text-white hover:bg-primaryd"
        >
          {copy.labels.start}
        </button>
      </p>
