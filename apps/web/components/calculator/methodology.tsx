...
export function Methodology({ view }: { view: CalculatorPageView }) {
  ...
  return (
    <Section id={CALCULATOR_ANCHORS.methodology} tone="white" labelledBy={headingId}>
      ...
      <div className="mt-16 border-t border-line pt-12" {...reveal()}>
          ...
          <table className="w-full min-w-[520px] border-collapse text-left text-[15px]">
            ...
            {view.rates.map((group) => (
              <tbody key={group.step}>
                <tr className="border-b border-line bg-mist2">
                  <th scope="colgroup" colSpan={2} className="py-3 pr-6 font-display font-bold text-ink">
                    {group.title}
                  </th>
                </tr>
                {group.rows.map((row) => (
                  <tr key={`${group.step}-${row.label}`} className="border-b border-line">
    ...
    <div className="glass rounded-2xl p-7">
      ...
      {view.tiers.length > 0 ? (
        <dl className="mt-5 space-y-4">
          {view.tiers.map((tier) => (
            <div key={tier.name} className="border-t border-white/15 pt-4 first:border-t-0 first:pt-0">
              ...
              <dd className="mt-1.5 text-[14px] leading-relaxed text-white/75">{tier.summary}</dd>
            </div>
          ))}
        </dl>
      ) : (
        <p className="mt-5 text-[15px] leading-relaxed text-white/75">{copy.bandsEmpty}</p>
      )}
      <p className="mt-6 border-t border-white/15 pt-4 text-[13.5px] leading-relaxed text-white/70">{copy.bandsNote}</p>
    </div>
  );
}
