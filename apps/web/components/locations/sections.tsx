...
export function LocationContactCard({ page }: { page: LocationDetailView }) {
  return (
    <div className="glass rounded-2xl p-7 text-[15px]" {...reveal(1)}>
      <p className="font-display text-[19px] font-bold">
        {page.address ? `Our ${page.city} office` : `Serving ${page.city}`}
      </p>
      {page.serviceArea ? <p className="mt-2.5 leading-relaxed text-white/80">{page.serviceArea}</p> : null}
      <dl className="mt-6 space-y-4 border-t border-white/15 pt-6">
        {page.address ? (
          <div>
            <dt className="text-[13px] text-white/65">Address</dt>
            <dd className="mt-1 whitespace-pre-line text-white">{page.address}</dd>
          </div>
        ) : null}
        <div>
          <dt className="text-[13px] text-white/65">Phone</dt>
          <dd className="mt-1">
            <a href={`tel:${page.contact.phoneE164}`} className="inline-flex items-center gap-2 py-1 font-semibold text-white hover:underline">
        ...
        <div>
          <dt className="text-[13px] text-white/65">Email</dt>
          <dd className="mt-1">
            <a href={`mailto:${page.contact.email}`} className="inline-block py-1 font-semibold break-all text-white hover:underline">
              {page.contact.email}
            </a>
          </dd>
        </div>
        ...
        {data.industries.length > 0 ? (
          <div className="lg:col-span-5" {...reveal(1)}>
            <div className="rounded-2xl border border-line bg-white p-7 lg:sticky lg:top-28">
              ...
              <ul className="mt-5 divide-y divide-line border-t border-line text-[16px] text-ink">
        ...
        <div className="grid gap-10 lg:grid-cols-12 lg:gap-20">
          ...
          <ul className="flex flex-wrap content-start gap-3 lg:col-span-5 lg:pt-3" {...reveal(1)}>
            {clients.names.map((name) => (
              <li key={name} className="rounded-xl border border-line bg-white px-5 py-3 font-display text-[18px] font-bold text-ink">
                {name}
              </li>
            ))}
          </ul>
    ...
    <Section id="how-we-work" tone="ink" labelledBy="how-we-work-heading">
      ...
      <ul className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {data.points.map((point, index) => (
          <li key={point.title} className="glass rounded-2xl p-7" {...reveal(index)}>
            <p className="font-display text-[15px] font-extrabold text-white/60">{String(index + 1).padStart(2, '0')}</p>
            <p className="mt-3 font-display text-[19px] leading-snug font-bold">{point.title}</p>
            <p className="mt-3 text-[15px] leading-relaxed text-white/75">{point.body}</p>
          </li>
        ))}
      </ul>
    </Section>
    ...
    <Section id="service-area" tone={tone} labelledBy="service-area-heading">
      <div className="grid items-center gap-12 lg:grid-cols-12 lg:gap-20">
        <div className={data.image ? 'lg:col-span-6' : 'lg:col-span-10'}>
          ...
          {data.places.length > 0 ? (
            <ul className="flex flex-wrap gap-2.5 text-[15px]" aria-label={`Places we cover around ${city}`}>
              {data.places.map((place) => (
                <li key={place} className="flex items-center gap-2 rounded-lg border border-line bg-white px-3.5 py-2 text-ink">
          ...
          {address ? (
            <div className="mt-8 border-t border-line pt-6" {...reveal(1)}>
    ...
    <li className="lift relative flex flex-col overflow-hidden rounded-2xl border border-line bg-white" {...reveal(step)}>
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <a
          ...
          className="mt-auto inline-block self-start pt-4 text-[14.5px] font-semibold text-primary after:absolute after:inset-0 hover:text-primaryd"
        >
